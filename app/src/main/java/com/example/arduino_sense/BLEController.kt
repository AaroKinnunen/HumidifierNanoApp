...
//                    btGattCharSpeed = null
            ...
            when (status) {
                BluetoothGatt.GATT_SUCCESS -> {
                    ...
                    if (characteristic == btGattCharHumidity) {
                        ...
                        /*
                        if(characteristic.value[0].toString().toInt()<=limitHum && data.getMode()== FanModes.AUTO){
                            data.setLedMode(LedMode.ON)
                            sendLEDData(data.getLedMode().to_arduino)
                        }
                        else if(characteristic.value[0].toString().toInt()>limitHum&& data.getMode()== FanModes.AUTO){
    ...
    fun sendLEDData(dataa: ByteArray?) {
            btGattCharLed.value = dataa
